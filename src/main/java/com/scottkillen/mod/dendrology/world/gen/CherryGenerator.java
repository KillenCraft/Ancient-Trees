...
public enum CherryGenerator implements IWorldGenerator
{
    INSTANCE;

    private final WorldGenAbstractTree treeGen = new Cherry(false);
    ...
    {
        if (!Settings.TreeGen.doCherryTreeGeneration()) return;
        ...
        if (isIdealHabitat(biomeTags)) generateCherry(world, random, chunkX, chunkZ);
    }

    private void generateCherry(World world, Random rand, int chunkX, int chunkZ)
    {
